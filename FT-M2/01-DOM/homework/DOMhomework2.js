...
// Tu código acá:
let title = document.getElementById("createdBy");
title.innerHTML = title.innerHTML + " Mauro"; //  title.innerHTML+= " Mauro"
...
// const todo1 = new ToDo("we") -> {description: "we", complete:false}
// todo1.completeToDo()
...
function buildToDo(todo, index) {
  // todo {description: "we", complete:false}   index 0
  // Tu código acá:
  var toDoShell = document.createElement("div"); // <div></>
  // toDoShell.class = "toDoShell"
  toDoShell.setAttribute("class", "toDoShell"); // <div class="toDoShell"></>
  var toDoText = document.createElement("span"); // <span></>
  toDoText.innerHTML = todo.description; //  <span>we</>
  toDoText.setAttribute("id", index); //  <span id="0">we</>
  if (todo.complete) {
    toDoText.setAttribute("class", "completeText"); //  <span class="completeText" id="0">we</>
  }
  toDoShell.appendChild(toDoText); // <div class="toDoShell"><span class="completeText" id="0">we</></>
  toDoText.addEventListener("click", completeToDo);
  return toDoShell; // {<div class="toDoShell"><span class="completeText" id="0">we</></>}
}
...
function buildToDos(toDos) {
  // [{description: "we", complete:false},{description: "tres", complete:false}]
  // Tu código acá:
  var array = toDos.map(function (element, index) {
    return buildToDo(element, index);
    //  [{<div class="toDoShell"><span class="completeText" id="0">we</></>}, {<div class="toDoShell"><span class="completeText" id="1">tres</></>}]
  });
  return array;
}
...
function displayToDos() {
  ...
  var result = buildToDos(toDoItems); // var toDoItems = [{description: "we", complete:false}]
  result.map(function (e) {
    if (e.children[0].innerHTML === "") {
      return false;
    } else {
      console.log("in e ", e.children[0].innerHTML)
...
// var toDoItems = [{description: "gracias", complete:false},{description: "pepe", complete:false}]
/*

function ToDo (description) { // description = ""
  // Tu código acá:
  this.description=description
  this.complete= false
}
...
function addToDo() {
  // Tu código acá:
  var inputActual = document.getElementById("toDoInput"); // document.querySelector("#..")
  var myTodo = new ToDo(inputActual.value); // {description: "", complete:false}
...
// ToDo.prototype.completeToDo = function(){
//   this.complete= true
// }  [{description: "gracias", complete:false},{description: "pepe", complete:false}]
function completeToDo(event) {
  // DESCOMENTAR LA SIGUIENTE LINEA
  const index = event.target.id;
  console.log(event);
  // Tu código acá:
  toDoItems[index].completeToDo(); // index=0 {description: "gracias", complete:true}
  displayToDos();
}
