...
function tipologiasSeparadas(arr) {
	let tipologiasRepetidas = []
	let tipologias = new Stack()
	if (arr.length === 0) return 'Tipologias inexistentes'
	for (let i = 0; i < arr.length; i++) {
		if (arr[i] === arr[i + 1]) {
			tipologiasRepetidas.push(arr[i])
			tipologiasRepetidas
		} else {
			tipologias.push(arr[i])
		}
	}
	for (let i = 0; i < tipologiasRepetidas.length; i++) {
		tipologias.push(tipologiasRepetidas[i])
	}
	if (tipologiasRepetidas.length === 0) {
